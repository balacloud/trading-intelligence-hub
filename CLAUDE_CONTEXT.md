...
### Fresh from Session 24 — Monday July 13 pre-flight (in priority order)
- [ ] **Confirm the Tradier token is functionally alive, not just ping-alive.** `/tradier/ping` succeeded Jul 10 (first time since ~Jul 5) but that only proves auth — run `test_tradier_calendar.py` or hit `/analyze/centaur` for a name with known earnings before trusting chain pulls or the earnings calendar.
- [ ] **Run the actual first live Scanner pass (PATH B) + Directional Builder + Centaur POST.** The watchlist was approved Session 24 but the run itself never executed — market was closed by the time the session resumed. This is the real gating event for the forward test's first entry, not the watchlist approval.
- [ ] **If gates produce survivors and/or near-miss rejects: log them per `FORWARD_TEST_PROTOCOL.md`** (`FWD_TEST:SURVIVOR|`/`FWD_TEST:REJECT|` prefix, quote-or-skip rule, `GET /journal/history` to capture the row id into `forward_test_log.csv`). Zero finalists is a valid, loggable stand-down — not a failure to retry.
