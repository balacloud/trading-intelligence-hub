...
- [ ] **Relay the `AGENTS.md` addition to Gemini's session** — the one-line "also read `GEMINI_STATE_HANDOFF.md` at session open" rule, drafted this session, not yet sent.
- [ ] **Relay the sharpened `IBKR_VERIFIED` finding to Gemini as its own pushback** ("return null, not a plausible fake" — see Known Issues row). Distinct from the already-fixed IVR>45 gate; not yet drafted as a standalone pushback message.
