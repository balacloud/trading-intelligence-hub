...
- [ ] **Bala: refresh the Tradier token** — now blocks TWO things: the TBLA verification (pending since Session 19) AND forward-test auto-marks via `/journal/monitor`. #1 blocker.
- [ ] **Relay batch to Gemini's session (three one-liners, send together):** (1) the `AGENTS.md` "read `GEMINI_STATE_HANDOFF.md` at session open" addition (drafted Session 20); (2) the `IBKR_VERIFIED` null-masking pushback (Known Issues row, Session 20); (3) NEW — "journal rows whose `setup_context` starts with `FWD_TEST:` are excluded from your performance tallies" (forward-test contamination guard).
- [ ] **Monday market-hours run does triple duty:** green-path Centaur validation (firing squeeze + monthly in 21–35 DTE; pre-screen CDE/KTOS/MP/UEC) + first forward-test survivor/reject log + first journal entries per `research/forward_test/FORWARD_TEST_PROTOCOL.md`. Confirm the daily ~10-min ritual actually fits before calling the forward test committed.
