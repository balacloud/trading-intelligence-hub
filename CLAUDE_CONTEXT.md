...
- [ ] **Daily marks begin for real next session, via Tradier (primary path):** 12 positions OPEN in Gemini's journal as of Jul 15, 2026 — AFRM (8), GDX (9), OKLO (10), NIO (18), CCJ (11), MP (12), URA (13), XLF (14), AVAV (15), WOLF (16), ECHO (17), INFQ (19). Mark daily via Tradier `/markets/quotes` + `/markets/options/chains` at bid/ask mid, close-of-day basis. IBKR MCP `get_option_data` is fallback only — confirmed intermittent within a single session (4/4 clean at 09:15 ET, 5/5 failed at 14:30 ET, same day). Never `/journal/monitor` (G1/G2, advisory only).
- [ ] **Send `HANDOFF_gemini_iv_hv_units.md` to Gemini's dev session.** Real contract bug: `app.py:762` expects `iv_hv_ratio` as a decimal fraction, the Directional Builder skill's documented convention produces a percentage-style number — every real Centaur submission following the skill's own docs would false-trigger `EDGE_VIOLATION`. Today's hub-side workaround (send as a fraction) is not a fix; needs Gemini's decision (fix their threshold to `>=100`, or fix the skill to divide by 100) and a re-verification per `skill-cross-repo-fix-verification.md` once done.
- [ ] **Bala decision, now three-times confirmed: cut or keep HIVE on CORE.** Sub-$1B finding confirmed a third independent time (this session's manual scanner paste), on top of the two live-MCP-pull confirmations from Sessions 24/26. Not a code fix — a watchlist-content call that's been open across three sessions.
...
- [x] ~~**Tradier's `fundamentals/calendars` endpoint returns 404**~~ — plan lacks Fundamentals Beta (confirmed Session 25); calendar-only, doesn't affect quotes/chains (confirmed Session 26). Hub skills' web-search-based earnings check remains the real source.
