...
### Fresh from Session 27 — next session pre-flight (in priority order)
- [ ] **Verify `HANDOFF_gemini_fwd_test_close_lockdown.md` once Bala pastes it into Gemini's dev session.** Asks Gemini to guard `/journal/close/<id>` and `/journal/update/<id>` against `FWD_TEST:` rows, forcing all closes through `/journal/resolve/<id>`. Real incident behind this (AFRM/AVAV closed off-protocol Jul 16 via the dashboard's native button) — don't skip the live re-verification once Gemini reports it fixed, same standard as everything else this session.
- [ ] **Run `resolve_positions.py` daily, once, near close** — the actual discipline now exists as a real script, not a manual conversation each time. 9 positions currently OPEN: XLF (21), JD (22), INFQ (19), NIO (18), WOLF (16), ECHO (17), CCJ (11), GDX (9), and the legacy XLF REJECT (14).
...
- [x] ~~**Send `HANDOFF_gemini_iv_hv_units.md` to Gemini's dev session**~~ — done and verified Session 27. Gemini chose `app.py:743` `>= 100.0` (percentage-style, consistent with `iv_rank`). Confirmed via live code read + running both test suites myself.
