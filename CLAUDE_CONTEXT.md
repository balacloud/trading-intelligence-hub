...
| HIGH (finding, unfixed) | Project has no version control | Fable 5 review, July 4 2026: flagged as the single largest architectural defect. Zero `git` history for a project whose two dominant documented risks are drift and stale summaries — `git diff` would replace much of the manual web-sync-audit apparatus. `git init` recommended before the next skill edit. |
...
- [ ] **Decide scope on the remaining Fable 5 review findings** — this session fixed only the 4 highest-severity bugs (user-selected "critical bugs only"). Everything else (no git, `OPTIONS_SIEVE_SPEC.md`, Scanner Gate C units, Radar's VIX source, watchlist sub-$1B risk, Trade Validator's dead couplings, SKILL_MAP.md/PERSONA.md staleness, the Pine script bugs) is logged in Known Issues above as "finding, unfixed" — pick the next batch.
- [ ] **`git init` this project** — flagged as the single highest-leverage fix in the review; not done yet (needs explicit go-ahead since it's a first-time repo action).
