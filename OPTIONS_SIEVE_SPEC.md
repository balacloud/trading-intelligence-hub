...
- **PATH B (Scanner):** `avg_90d_usd_volume` pulled directly from MCP. **✅ RESOLVED (settled twice with live data — Jul 13 and Jul 18, 2026):** confirmed a genuine **daily average**, not a 90-day total. Live cross-check (Session 28): NVDA returned $31.7B (only plausible as a daily figure — 153M shares/day at $207.40, consistent with NVDA's real trading volume; a 90-day-total reading would imply ~$503M/day, implausibly low). HIVE returned $78.5M, matching the $83.2M/day figure already recorded live when HIVE failed this exact gate in Session 27 — two independent pulls, days apart, in the same range. The $100M/day threshold is correctly scaled on both paths; no recalibration needed.
