...
- **Target / stop (mechanical, both groups):** stop = entry mid × 0.70 (the system's standard 30% hard stop). **Target changed Session 27 (Jul 16, 2026, Bala's explicit call): entry mid × 1.80 (+80% profit-take), replacing the prior × 1.60 (+60%).** Not retroactive — the 12 positions logged before Jul 16 keep their original × 1.60 target as stored in Gemini's journal at log time (no DB rows edited); every position logged from Jul 16 onward uses × 1.80. The resolution script never recomputes a target — it always reads each trade's own stored `target_price`/`stop_loss`, so old and new positions are handled correctly without special-casing. Gemini's synthesized levels, if any, are recorded as metadata only and never drive resolution.
