...
### Guardrail 7 — Regime-aware is not the same claim as regime-gated (raised by Bala, Jul 28 2026, Session 37)

Bala asked directly whether this pipeline is "100% regime gated... trading with the market, not against it." Checked against the live code before answering, not assumed: **no gate, sieve, or entry/direction decision currently branches on any regime signal.** Three distinct things travel under the word "regime" in this project, and they should not be conflated:

1. **VIX-level regime (STANDARD/HIGH-FEAR)** — fetched live every `build_and_log.py` run, persisted per-trade (Guardrail 3). Logged only. Measures market-wide fear/volatility, not direction — a HIGH-FEAR reading says nothing about whether the market is trending up or down.
2. **Broad-market trend regime (e.g. SPY vs its own 200-day SMA)** — not computed or tracked anywhere in this pipeline today, for any trade, at all. If "trading with the market" means "don't buy calls while the S&P itself is in a confirmed downtrend," that is a new signal, not a dormant one — nothing to gate on exists yet.
3. **Per-name trend alignment (this ticker's own SMA200 vs the direction of the option bought)** — already computed and logged every trade (`trend_200d`), and already the exact open hypothesis in `FORKING_PATHS_LOG.md` Entries 4/5/6: does a call bought into an UPTREND / a put bought into a DOWNTREND actually perform better than the opposite, across the full resolved set? At n=25 the UPTREND/DOWNTREND split is 22%/54% — directionally consistent with "respect the trend," but not close to locked, and it moved a lot (0%/64% → 22%/54%) the moment n grew by 7. Gemini's own "Senior Partner" proposal (Entry 5) is this same idea from an independent source.

**Why this isn't being wired in today:** turning #3 into an actual hard gate (skip/flip any trade where the mechanical direction vote contradicts `trend_200d`) mid-test is a pipeline-rule change. Guardrail 1 already bans exactly this — changing a gate because an interim pattern "looks convincing" — no matter how intuitively appealing "don't fight the trend" sounds. It would only be legitimate under Guardrail 5(c): Bala's explicit judgment call, made deliberately, not folded into a conversation about something else. This entry exists so that call can be made on purpose, later, with eyes open — not defaulted into.

**What to revisit, and when:** once Entry 6's split has enough resolved trades in the smaller bucket to say something (n=9 UPTREND today, per Guardrail 5 this is still thin), decide explicitly whether to (a) lock per-name trend-alignment as a real hard gate, (b) keep it as judgment-layer scrutiny only (Gemini's own proposed posture, already endorsed in Entry 5/6), or (c) build #2 (broad-market trend) as a genuinely new signal if that's actually what "trading with the market" was meant to capture. Bala should clarify which of #1/#2/#3 he means before any of this becomes code — they point at different fixes.
